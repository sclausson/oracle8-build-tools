...
#==========
RUN mkdir -p /opt/jdk
RUN wget --header "Cookie: oraclelicense=accept-securebackup-cookie" -O /opt/jdk/jdk-8u60-linux-x64.tar.gz http://download.oracle.com/otn-pub/java/jdk/8u60-b27/jdk-8u60-linux-x64.tar.gz
RUN tar -zxf /opt/jdk/jdk-8u60-linux-x64.tar.gz -C /opt/jdk
RUN update-alternatives --install /usr/bin/java java /opt/jdk/jdk1.8.0_60/bin/java 100 \
 && update-alternatives --install /usr/bin/javac javac /opt/jdk/jdk1.8.0_60/bin/javac 100 \
 && rm /opt/jdk/jdk-8u60-linux-x64.tar.gz
ENV JAVA_HOME /opt/jdk/jdk1.8.0_60
...
#=======================
# Special Extenda stuff
#=======================
RUN mkdir -p /var/opt/extenda \
  && chmod 777 /var/opt/extenda \
  && mkdir -p /data/sco \
  && chmod 777 /data/sco \
  && wget -O $JAVA_HOME/jre/lib/ext/openjfx-monocle-1.8.0_60.jar http://repository.extenda.local/nexus/service/local/repositories/releases/content/org/testfx/openjfx-monocle/1.8.0_60/openjfx-monocle-1.8.0_60.jar
